...
switch ($action) {
	case 'dang-nhap':
		...
		if (isset($_SESSION[$login_member]['active']) && $_SESSION[$login_member]['active'] == true) $func->transfer("Trang không tồn tại", $config_base . 'account/thong-tin', false);
	...
	case 'khoahoc_chitiet':
		...
		$template = "account/khoahoc_chitiet";
		$title_crumb = 'Chi tiết bài học';
		$id_baikhoahoc = openssl_decrypt($_REQUEST['id_baikhoahoc'], 'aes-256-cbc', 'art', 0, 'art');
		// if (isset($_POST['baikhoahoc'])) {
		// 	baikhoahoc_update($id_baikhoahoc);
		// }
		baikhoahoc_update($id_baikhoahoc);
		break;
	...
	case 'khoahoc-yeuthich':
		if (!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại", $config_base, false);
		$template = "account/khoahocyeuthich";
		$title_crumb = "Khóa học yêu thích";
		info_khoahocyeuthich();
		break;

	case 'lichsu-muahang':
		if (!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại", $config_base, false);
		$template = "account/lichsumuahang";
		$title_crumb = "Lịch sử mua hàng";
		info_lichsumuahang();
		break;

	case 'course':
		if (!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại", $config_base, false);
		$template = "account/khoahoc";
...
function info_lichsumuahang()
{
	global $d, $func, $row_detail, $order_lsmh, $config_base, $login_member;

	$iduser = $_SESSION[$login_member]['id'];

	if ($iduser) {
		$row_detail = $d->rawQueryOne("select * from #_member where id = ? limit 0,1", array($iduser));
		$order_lsmh = $d->rawQuery("select * from #_order where id_user = '" . $iduser . "'");
	} else {
		$func->transfer("Trang không tồn tại", $config_base, false);
	}
}

function info_khoahocyeuthich()
{
	global $d, $func, $row_detail, $khoahocyeuthich, $config_base, $login_member;

	$iduser = $_SESSION[$login_member]['id'];

	if ($iduser) {
		$row_detail = $d->rawQueryOne("select * from #_member where id = ? limit 0,1", array($iduser));
		$khoahocyeuthich = $d->rawQuery("select * from #_member_yeuthich where id_user = '" . $iduser . "'");
	} else {
		$func->transfer("Trang không tồn tại", $config_base, false);
	}
}
...
function info_khoahoc()
{
	global $d, $func, $row_detail, $config_base, $login_member, $order_lsmh;
...
function baikhoahoc_update($id_baikhoahoc)
{
	global $d, $func, $config_base, $row_detail, $baikhoahoc, $chuong_khoahoc, $khoahoc, $member_khoahoc_update, $login_member;
	...
	$baikhoahoc = $d->rawQueryOne("select * from #_product_doday where id = '" . $id_baikhoahoc . "'");
	$chuong_khoahoc = $d->rawQueryOne("select * from #_product_brand where id = '" . $baikhoahoc['id_chuong_khoahoc'] . "'");
	$khoahoc = $d->rawQueryOne("select * from #_product where id = '" . $baikhoahoc['id_khoahoc'] . "'");
	$member_khoahoc_update = $d->rawQueryOne("select * from #_member_khoahoc where id_user = '".$iduser."' and id_bai_khoahoc = '" . $baikhoahoc['id'] . "'");
	...
	if (isset($_POST['baikhoahoc'])) {
		...
		$data['id_chuong'] = $_POST['id_chuong'];
		$data['id_khoahoc'] = $_POST['id_khoahoc'];
		$data['id_bai_khoahoc'] = $_POST['id_bai_khoahoc'];
		$data['id_user'] = $iduser;

		$bai_khoahoc = $_POST['id_bai_khoahoc'];
		$khoahoc_update = $d->rawQueryOne("select * from #_member_khoahoc where id_user = '".$data['id_user']."' and id_bai_khoahoc = '" . $data['id_bai_khoahoc'] . "'");
		// var_dump(!empty($khoahoc_update['photo']));
		if(!empty($khoahoc_update['photo'])){
			$data['photo'] = $khoahoc_update['photo'] .'_'.$data['photo'];
		}
		// var_dump($data);die();

		if(!empty($khoahoc_update)){
			if ($d->update('member_khoahoc', $data)) {
				$func->transfer("Nộp bài thành công", $config_base . 'account/khoahoc_chitiet?id_baikhoahoc=' . openssl_encrypt($bai_khoahoc, 'aes-256-cbc', 'art', 0, $iv = 'art') . '');
			} else {
				$func->transfer("Nộp bài không thành công", $config_base . 'account/khoahoc_chitiet?id_baikhoahoc=' . openssl_encrypt($bai_khoahoc, 'aes-256-cbc', 'art', 0, $iv = 'art') . '', false);
			}
		}else{
			if ($d->insert('member_khoahoc', $data)) {
				$func->transfer("Nộp bài thành công", $config_base . 'account/khoahoc_chitiet?id_baikhoahoc=' . openssl_encrypt($bai_khoahoc, 'aes-256-cbc', 'art', 0, $iv = 'art') . '');
			} else {
				$func->transfer("Nộp bài không thành công", $config_base . 'account/khoahoc_chitiet?id_baikhoahoc=' . openssl_encrypt($bai_khoahoc, 'aes-256-cbc', 'art', 0, $iv = 'art') . '', false);
			}
...
function info_thongbao()
{
	global $d, $func, $row_detail, $config_base, $login_member, $thongbao_user;
	...
	$thongbao_user = $d->rawQuery("select * from #_thongbao_user where id_user = '$iduser' order by id desc");
	// var_dump("select * from #_thongbao_user where id_user = '$iduser' order by id desc");
}
...
function login()
{
	...
	if ($row['id']) {
		if ($row['password'] == $passwordMD5) {
			...
			$func->transfer("Đăng nhập thành công", $config_base . 'account/thong-tin');
		} else {
...
function google_client_user()
{
	...
	if (isset($_GET['code'])) {
		...
		if (isset($check['error'])) {
			...
			$info = $func->checkThongTin($user->email);
