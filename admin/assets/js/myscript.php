<script>
	...
	$(document).ready(function() {
		...
		$('body').on('change', '.filer-khoahoc', function() {
			var id = $(this).val();
			var table = $(this).data('table');
			var type = $(this).data('type');

			$.ajax({
				url: 'ajax/ajax_chuongkhoahoc.php',
				type: 'POST',
				data: {
					id: id,
					table: table,
				...
				success: function(result) {
					var op = "<option value='0'>Chọn chương khóa học</option>";

					$("#id_chuong_khoahoc").html(result);
				}
			});
		});
		...
		$('body').on("click", ".create-faq", function() {
			...
			// Gọi AJAX để lấy HTML của phần upload ảnh
			$.ajax({
				url: "ajax/get_single_image_faq.php", // File PHP trả về HTML ảnh
				type: "POST",
				data: {
					soluong: soluong
				},
				success: function(singleImageHtml) {
					var newFaqHtml = `
                <div class="all_faq_content mb-3">
                    <div class="form-group">
                        <div class="label-seo">
                            <label for="titlevi${soluong}">Đạt được title (vi):</label>
                        </div>
                        <input type="text" class="form-control check-seo title-seo" name="datafaq${soluong}[tenvi]" id="titlevi${soluong}" placeholder="Đạt được Title (vi)" value="">
                    </div>
                    <div class="form-group">
                        <div class="label-seo">
                            <label for="noidungvi${soluong}">Đạt được Description (vi):</label>
                        </div>
                        <textarea class="form-control check-seo description-seo ckeditor" name="datafaq${soluong}[noidungvi]" id="noidungvi${soluong}" rows="5" placeholder="Đạt được nội dung (vi)"></textarea>
                    </div>
                    ${singleImageHtml}
                </div>
            `;

					$('.tab_pane_faq').append(newFaqHtml);

					// Gọi lại script để kích hoạt ô input file
					reinitializeFileUpload();
				}
			});
		});

		// Hàm khởi động lại sự kiện click của input file
