...
switch ($act) {
	...
	case "edit_khoahoc":
		...
		save_khoahoc_item();
		$template = "user/khoahoc/item_add";
		break;
...
/* Khóa học user */
function get_khoahoc_item()
{
	global $d, $func, $curPage, $items, $user, $paging, $config, $khoahoc, $chuong, $user_text;

	$id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;
	$id_chuong = (isset($_GET['id_chuong'])) ? htmlspecialchars($_GET['id_chuong']) : 0;
	$id_khoahoc = (isset($_GET['id_khoahoc'])) ? htmlspecialchars($_GET['id_khoahoc']) : 0;
	...
	$user = $id;
	$user_text = $d->rawQueryOne("select * from #_member where id = '$user'");
	if (!empty($id_chuong)) {
		$khoahoc = $id_khoahoc;
		$chuong = $id_chuong;
		$sql = "select * from #_product_doday where id_khoahoc = '$id_khoahoc' and id_chuong_khoahoc = '$id_chuong' order by stt,id desc $limit";
	} else if (!empty($id_khoahoc)) {
		$khoahoc = $id_khoahoc;
		$sql = "select * from #_product_brand where id_khoahoc = '$id_khoahoc'  order by stt,id desc $limit";
	} else {
		$sql = "SELECT tode.* FROM table_order_detail tode JOIN table_order taod ON tode.id_order = taod.id WHERE taod.id_user = '$id' order by id desc $limit";
		// $sql = "select * from #_member where id <> 0 $where order by stt,id desc $limit";
	}
	...
	$count = $d->rawQueryOne($sqlNum);
	$total = $count['num'];
	$url = "index.php?com=user&act=man_khoahoc";
	$paging = $func->pagination($total, $per_page, $curPage, $url);
}

/* Save visitor */
function save_khoahoc_item()
{
	global $d, $func, $curPage, $item;

	$id_user = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;
	$id_chuong = (isset($_GET['id_chuong'])) ? htmlspecialchars($_GET['id_chuong']) : 0;
	$id_khoahoc = (isset($_GET['id_khoahoc'])) ? htmlspecialchars($_GET['id_khoahoc']) : 0;
	$id_bai = (isset($_GET['id_bai'])) ? htmlspecialchars($_GET['id_bai']) : 0;
	$item = $d->rawQueryOne("select * from #_member_khoahoc where id_user = '$id_user' and id_khoahoc = '$id_khoahoc' and id_chuong = '$id_chuong' and id_bai_khoahoc = '$id_bai' order by id desc");

	if (!empty($_POST)) {
		$id = htmlspecialchars($_POST['id']);
		$id_user = htmlspecialchars($_POST['id_user']);
		$id_khoahoc = htmlspecialchars($_POST['id_khoahoc']);
		$id_chuong = htmlspecialchars($_POST['id_chuong']);
		$id_bai = htmlspecialchars($_POST['id_bai']);
		// $id_user = htmlspecialchars($_POST['id_user']);
		...
		if ($data) {
			...
			$data['check'] = (isset($data['check'])) ? 1 : 0;
		}
		...
		if ($id) {
			...
			if ($d->update('member_khoahoc', $data)) {
				...
				$user_kh = $d->rawQueryOne("select tenvi from #_product where id = '$id_khoahoc'");
				$user_chuong = $d->rawQueryOne("select tenvi from #_product_brand where id = '$id_chuong'");
				$user_bai = $d->rawQueryOne("select tenvi from #_product_doday where id = '$id_bai'");

				$data_thongbao['id_user'] = $id_user;
				$data_thongbao['id_khoahoc'] = $id_khoahoc;
				$data_thongbao['id_chuong'] = $id_chuong;
				$data_thongbao['id_bai_khoahoc'] = $id_bai;
				$data_thongbao['ten'] = 'Cập nhật thông tin';
				$data_thongbao['noidung'] = $user_bai['tenvi'].' thuộc '.$user_chuong['tenvi'].' thuộc khóa học '.$user_kh['tenvi'].' đã được cập nhật thông tin ';
				$data_thongbao['ngaytao'] = time();

				// var_dump($data_thongbao);
				// var_dump($d->insert('thongbao_user', $data_thongbao));die();
				$d->insert('thongbao_user', $data_thongbao);

				$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=user&act=man_khoahoc&id=" . $id_user . "&id_khoahoc=" . $id_khoahoc . "&id_chuong=" . $id_chuong . "&id_bai=" . $id_bai . "&p=" . $curPage);
			}
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=user&act=man_khoahoc&id=" . $id_user . "&id_khoahoc=" . $id_khoahoc . "&id_chuong=" . $id_chuong . "&id_bai=" . $id_bai . "&p=" . $curPage, false);
		} else {

			if ($d->insert('member_khoahoc', $data)) $func->transfer("Lưu dữ liệu thành công", "index.php?com=user&act=man_khoahoc&id=" . $id_user . "&id_khoahoc=" . $id_khoahoc . "&id_chuong=" . $id_chuong . "&id_bai=" . $id_bai . "&p=" . $curPage);
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=user&act=man_khoahoc&id=" . $id_user . "&id_khoahoc=" . $id_khoahoc . "&id_chuong=" . $id_chuong . "&id_bai=" . $id_bai . "&p=" . $curPage, false);
		}
	}
}
