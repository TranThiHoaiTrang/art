...
function get_giangvien($id = 0)
{
	global $d, $type;

	if ($id) {
		$temps = $d->rawQueryOne("select id_giangvien from #_product where id = ? and type = ? limit 0,1", array($id, $type));
		$arr_giangvien = explode(',', $temps['id_giangvien']);

		for ($i = 0; $i < count($arr_giangvien); $i++) $temp[$i] = $arr_giangvien[$i];
	}
	$row_giangvien = $d->rawQuery("select tenvi, id from #_news where type = 'giangvien' order by stt,id desc");

	$str = '<select id="giangvien_group" name="giangvien_group[]" class="form-control select2" >';
	$str .= '<option value="0"> Chọn giảng viên</option>';
	for ($i = 0; $i < count($row_giangvien); $i++) {
		if (isset($temp) && count($temp) > 0) {
			if (in_array($row_giangvien[$i]['id'], $temp)) $selected = 'selected="selected"';
			else $selected = '';
		} else {
			$selected = '';
		}
		$str .= '<option value="' . $row_giangvien[$i]["id"] . '" ' . $selected . ' > ' . $row_giangvien[$i]["tenvi"] . '</option>';
	}
	$str .= '</select>';
			...
			<div class="<?= $colLeft ?>">
				<div id="scroll-left">
					...
					<div class="card card-primary card-outline text-sm">
									...
									<div class="tab-content" id="custom-tabs-three-tabContent-lang">
										...
										<?php foreach ($config['website']['lang'] as $k => $v) { ?>
											<div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
												...
												<?php } ?>
												<?php if (isset($config['product'][$type]['amthanh']) && $config['product'][$type]['amthanh'] == true && $k == 'vi') { ?>
													<div class="form-group">
														<label for="amthanh">Âm thanh:</label>
														<input type="text" class="form-control for-seo" name="data[amthanh]" id="amthanh" placeholder="Nhập Âm thanh" value="<?= @$item['amthanh'] ?>">
													</div>
												<?php } ?>
												<?php if (isset($config['product'][$type]['congcu']) && $config['product'][$type]['congcu'] == true && $k == 'vi') { ?>
													<div class="form-group">
														<label for="congcu">Công cụ:</label>
														<input type="text" class="form-control for-seo" name="data[congcu]" id="congcu" placeholder="Nhập Công cụ" value="<?= @$item['congcu'] ?>">
													</div>
												<?php } ?>
												<?php if (isset($config['product'][$type]['dichvu']) && $config['product'][$type]['dichvu'] == true && $k == 'vi') { ?>
													<div class="form-group">
														<label for="dichvu">Dịch vụ:</label>
														<input type="text" class="form-control for-seo" name="data[dichvu]" id="dichvu" placeholder="Nhập Dịch vụ" value="<?= @$item['dichvu'] ?>">
													</div>
												<?php } ?>
												<?php if (isset($config['product'][$type]['videobaigiang']) && $config['product'][$type]['videobaigiang'] == true && $k == 'vi') { ?>
													<div class="form-group">
														<label for="videobaigiang">Số video bài giảng:</label>
														<input type="text" class="form-control for-seo" name="data[videobaigiang]" id="videobaigiang" placeholder="Nhập số video bài giảng" value="<?= @$item['videobaigiang'] ?>">
													</div>
												<?php } ?>
												<?php if (isset($config['product'][$type]['thoigianhoc']) && $config['product'][$type]['thoigianhoc'] == true && $k == 'vi') { ?>
													<div class="form-group">
														<label for="thoigianhoc">thời gian học:</label>
														<input type="text" class="form-control for-seo" name="data[thoigianhoc]" id="thoigianhoc" placeholder="Nhập thời gian học" value="<?= @$item['thoigianhoc'] ?>">
													</div>
												<?php } ?>
												<?php if (isset($config['product'][$type]['thanhpham']) && $config['product'][$type]['thanhpham'] == true && $k == 'vi') { ?>
													<div class="form-group">
														<label for="thanhpham">số lượng thành phẩm:</label>
														<input type="text" class="form-control for-seo" name="data[thanhpham]" id="thanhpham" placeholder="Nhập số lượng thành phẩm" value="<?= @$item['thanhpham'] ?>">
													</div>
												<?php } ?>

												<?php if (isset($config['product'][$type]['chuongtrinhhoc']) && $config['product'][$type]['chuongtrinhhoc'] == true) { ?>
													<div class="form-group">
														<label for="chuongtrinhhoc<?= $k ?>">Chương trình học (<?= $k ?>):</label>
														<textarea class="form-control for-seo <?= (isset($config['product'][$type]['chuongtrinhhoc_cke']) && $config['product'][$type]['chuongtrinhhoc_cke'] == true) ? 'ckeditor' : '' ?>" name="data[chuongtrinhhoc<?= $k ?>]" id="chuongtrinhhoc<?= $k ?>" rows="5" placeholder="Chương trình học (<?= $k ?>)"><?= htmlspecialchars_decode(@$item['chuongtrinhhoc' . $k]) ?></textarea>
													</div>
												<?php } ?>
						...
						<?php if (isset($config['product'][$type]['datduoc']) && $config['product'][$type]['datduoc'] == true) { ?>
							<div class="card card-primary card-outline text-sm ">
								<div class="card-header">
									<h3 class="card-title">Những gì đạt được sau khóa học</h3>
								</div>
								<div class="card-body">
									<?php
				...
				<div id="scroll-right">
							...
							<div class="card-body">
								<div class="form-group-category row">
									...
									<!-- </?php if (isset($config['product'][$type]['brand']) && $config['product'][$type]['brand'] == true) { ?>
										<div class="form-group col-xl-6 col-sm-6">
											<label class="d-block" for="id_brand">Chương:</label>
											<?= $func->get_ajax_category('product', 'brand', $type, 'Chọn Chương') ?>
										</div>
									</?php } ?> -->
									...
									<?php if (isset($config['product'][$type]['giangvien']) && $config['product'][$type]['giangvien'] == true) { ?>
										<div class="form-group col-xl-6 col-sm-6">
											<label class="d-block" for="id_giangvien">Giảng viên:</label>
											<?= get_giangvien(@$item['id']) ?>
										</div>
									<?php } ?>
					...
					<?php if (isset($flagGallery) && $flagGallery == true) { ?>
						<div class="card card-primary card-outline text-sm ">
							...
							<div class="card-header">
								<h3 class="card-title">Bộ sưu tập khóa học</h3>
								<div class="card-tools">
									<button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
								</div>
							</div>
