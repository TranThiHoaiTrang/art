...
class Functions
{
	...
	function generateImage($text) {
		...
		$bgColor = imagecolorallocate($img, 0, 102, 204); // Màu nền xanh
		$textColor = imagecolorallocate($img, 255, 255, 255); // Màu chữ trắng
	
		imagefilledrectangle($img, 0, 0, 100, 100, $bgColor);
		$font ='../fonts/SVN-Poppins/TTF/SVN-Poppins-SemiBold.ttf'; // Đường dẫn tới font chữ (cần có font .ttf)
		imagettftext($img, 30, 0, 25, 60, $textColor, $font, $text);
	...
	function getShortInitials($name) {
		$words = explode(' ', trim($name)); // Tách tên thành mảng các từ
		if (!empty($words)) {
			return strtoupper(mb_substr(end($words), 0, 1)); // Lấy chữ cái đầu của từ cuối
		}
		return ''; // Trả về rỗng nếu không có tên
	}
	...
	/* Get category by link */
	public function get_khoahoc($table = '', $level = '', $type = '', $title_select = 'Chọn khóa học')
	{
		$where = '';
		$params = array($type);
		$id_parent = 'id_khoahoc';

		$title_select = 'Chọn khóa học';
		

		$rows = $this->d->rawQuery("select tenvi, id from #_" . $table . " where type = ? " . $where . " order by stt,id desc", $params);
		// var_dump("select tenvi, id from #_".$table."_".$level." where type = ? ".$where." order by stt,id desc",$params);
		$str = '<select id="' . $id_parent . '" name="' . $id_parent . '" data-type="'.$type.'" data-table="#_product_brand" class="form-control filer-khoahoc select2"><option value="0">' . $title_select . '</option>';
		foreach ($rows as $v) {
			if (isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
			else $selected = "";
	...
	public function get_khoahoc_chuong($table = '', $level = '', $type = '', $title_select = 'Chọn chương khóa học')
	{
		$where = '';
		$params = array($type);
		$id_parent = 'id_chuong_khoahoc';

		$title_select = 'Chọn chương khóa học';
		if($_REQUEST['id_khoahoc']){
			$idkhoahoc = (isset($_REQUEST['id_khoahoc'])) ? htmlspecialchars($_REQUEST['id_khoahoc']) : 0;
			$rows = $this->d->rawQuery("select tenvi, id from #_" . $table . " where id_khoahoc = '$idkhoahoc' and type = ? " . $where . " order by stt,id desc", $params);
		}else{
			$rows = $this->d->rawQuery("select tenvi, id from #_" . $table . " where type = ? " . $where . " order by stt,id desc", $params);
		}
		// var_dump("select tenvi, id from #_".$table."_".$level." where type = ? ".$where." order by stt,id desc",$params);
		$str = '<select id="' . $id_parent . '" name="' . $id_parent . '" class="form-control filer-khoahocchuong select2"><option value="0">' . $title_select . '</option>';
		foreach ($rows as $v) {
			if (isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
			else $selected = "";
	...
	function clientGoogle()
	{
		...
		$client_id = '998798557545-qnmr7id29j689iqeu0hnnr4oq86cvi83.apps.googleusercontent.com'; // Client ID
		$client_secret = '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'; // Client secret
		$redirect_uri = $config_base . 'account/google';
	...
	function checkThongTin($username, $rule = '')
	{
		...
		if ($rule == '') {
			$row = $d->rawQueryOne("select * from #_member where email = ? limit 0,1", array($username));
			// $row='SELECT * FROM `member` WHERE `username` = ?';

			// ngược lại kiểm tra theo id

		} else {

			$row = 'SELECT * FROM `member` WHERE `id` = ?';
		}
